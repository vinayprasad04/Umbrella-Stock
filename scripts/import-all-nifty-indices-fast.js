...
async function importAllNiftyIndices() {
  try {
    ...
    console.log('🚀 Starting import of all Nifty indices (optimized)...\n');

    for (const { file, index } of NIFTY_INDEX_FILES) {
      console.log(`📊 Processing: ${index} - ${path.basename(file)}`);
      ...
      if (symbols.length === 0) {
        summary.byIndex[index] = { found: 0, updated: 0 };
        continue;
      }

      // Step 1: Remove this index from all stocks (bulk operation)
      const removeResult = await collection.updateMany(
        { 'additionalInfo.niftyIndices': index },
        { $pull: { 'additionalInfo.niftyIndices': index } }
      );

      // Step 2: Add index to stocks in the CSV (bulk operation)
      ...
      summary.total += updateResult.modifiedCount;
    }
    ...
    for (const [index, stats] of Object.entries(summary.byIndex)) {
      if (stats.skipped) {
        ...
        console.log(`${index}: ${stats.updated} stocks updated`);
      }
    }
    ...
    // Verify counts
    console.log('\n📊 Final verification - Stock counts by index:');
