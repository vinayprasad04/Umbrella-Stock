...
export default function StockNews({ symbol }: StockNewsProps) {
  ...
  const fetchNews = async (page: number) => {
    try {
      ...
      const response = await axios.get(
        `/api/stocks/${symbol}/activities?page=${page}&limit=${ITEMS_PER_PAGE}&type=news-article`
      );
  ...
  if (loading && currentPage === 1) {
    return (
      <div className="card" id="stock-news-section">
        <div className="flex items-center justify-center py-12">
          <LoadingSpinner size="lg" />
        </div>
        <p className="text-center text-gray-600 mt-4">Loading news...</p>
      </div>
    );
  }
    ...
    return (
      <div className="card" id="stock-news-section">
        <div className="flex items-center mb-6">
          ...
          <h3 className="text-xl font-bold text-gray-900">Latest News & Updates</h3>
        </div>
        <div className="text-center py-12">
          <div className="text-6xl mb-4">📭</div>
          <h4 className="text-lg font-semibold text-gray-900 mb-2">No News Available</h4>
          <p className="text-gray-600">
            There are no news articles available for {symbol} at the moment.
          </p>
        </div>
      </div>
    );
