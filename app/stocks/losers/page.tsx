...
export default function TopLosersPage() {
  ...
  const { data: losers, isLoading, error, refetch } = useQuery({
    queryKey: ['topLosers', limit],
    queryFn: async () => {
      const response = await axios.get(`/api/stocks/top-losers?limit=${limit}`);
      if (!response.data.success) {
        throw new Error(response.data.error || 'Failed to fetch top losers');
      }
      ...
      <main className="w-full max-w-[1600px] mx-auto px-6 py-12">
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              onClick={() => refetch()}
              className="flex items-center px-4 py-2 bg-red-600 text-white rounded-xl font-medium hover:bg-red-700 transition-all duration-300"
            >
              <span className="mr-2">🔄</span>
              Refresh Data
            </button>
          ...
          <div className="text-center">
            <div className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-red-500/10 to-rose-600/10 rounded-full text-sm font-medium text-gray-700 mb-6 backdrop-blur-sm border border-red-200/50">
              <div className="w-2 h-2 rounded-full mr-2 bg-red-400"></div>
              Live Market Data • Updated every 30 seconds
            </div>
            
            <h1 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-red-600 via-rose-700 to-red-800 mb-4 leading-tight">
              Top Losers
            </h1>
            <p className="text-xl text-gray-600 mb-8 max-w-2xl mx-auto leading-relaxed">
              Monitor the worst performing stocks in the market today and identify potential opportunities or risks
            </p>
        ...
        {losers && losers.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
            <div className="bg-white/70 backdrop-blur-md rounded-2xl p-6 border border-white/50 shadow-lg text-center">
              ...
              <div className="text-sm text-gray-600">Total Losers</div>
            </div>
            <div className="bg-white/70 backdrop-blur-md rounded-2xl p-6 border border-white/50 shadow-lg text-center">
              <div className="text-2xl font-bold text-red-600 mb-2">
            ...
            <div className="bg-white/70 backdrop-blur-md rounded-2xl p-6 border border-white/50 shadow-lg text-center">
              <div className="text-2xl font-bold text-red-600 mb-2">
                {(losers.reduce((sum, stock) => sum + stock.changePercent, 0) / losers.length).toFixed(2)}%
              </div>
              <div className="text-sm text-gray-600">Average Loss</div>
            </div>
            <div className="bg-white/70 backdrop-blur-md rounded-2xl p-6 border border-white/50 shadow-lg text-center">
              <div className="text-2xl font-bold text-red-600 mb-2">
                ₹{Math.max(...losers.map(s => s.volume)).toLocaleString('en-IN')}
              </div>
        ...
        <div className="bg-white/70 backdrop-blur-md rounded-3xl p-8 border border-white/50 shadow-xl">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-12 h-12 bg-gradient-to-r from-red-500 to-rose-600 rounded-2xl flex items-center justify-center shadow-lg">
              <span className="text-white text-xl">📉</span>
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Market Decliners</h2>
              <p className="text-sm text-gray-600">Stocks with highest percentage losses today</p>
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">No Losers Found</h3>
              <p className="text-gray-600">No stocks are showing negative performance at this time.</p>
            </div>
          )}
        </div>
        ...
        {losers && losers.length > 0 && (
          <div className="mt-12 text-center">
            <div className="bg-gradient-to-r from-red-50 to-rose-50 rounded-2xl p-8 border border-red-100">
